...
from infrastructure.adapters.gcp.oauth_callback_handler import InMemoryOAuthCallbackHandler
    ...
    async def wait_for_user_authentication(self, timeout: int = 300) -> AuthConfig:
        ...
        while elapsed < timeout:
            ...
            if code:
                ...
                self.auth_config.exchanged_auth_credential.oauth2.auth_response_uri = auth_response_uri
                self.auth_config.exchanged_auth_credential.oauth2.redirect_uri = self.redirect_uri
                return self.auth_config

            await asyncio.sleep(self.poll_interval)
            elapsed += self.poll_interval

        raise TimeoutError(f"Authentication timeout after {timeout}s for state: {state}")
...
class AuthInterceptor:
    ...
    async def intercept_auth_event(self, event: Event) -> AsyncGenerator[Event, None]:
        ...
        if not (function_call_id := auth_request_function_call.id):
            raise ValueError(f"Cannot get function call id from: {auth_request_function_call}")

        if not auth_request_function_call.args or not (
                auth_config := auth_request_function_call.args.get('authConfig')):
            raise ValueError(f"Cannot get auth config from: {auth_request_function_call}")
        ...
        updated_auth_config = await self.auth_config_handler.wait_for_user_authentication()
        ...
        while credentials_needed or first_call:
            ...
            async for event in self.runner.run_async(
                ...
                if self.auth_interceptor.is_auth_event(event):
                    ...
                    async for auth_event in self.auth_interceptor.intercept_auth_event(event):
